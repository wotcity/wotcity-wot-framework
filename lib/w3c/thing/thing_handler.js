//
// This code is came from W3C Web of Things Framework for NodeJS
...
function register_thing (name, thing) {
    ...
    console.log('Register thing: ' + name);
...
function init(things) {
    try {
        if (!things || !util.isArray(things)) {
            return console.log("Error in parsing things configuration file. Invalid data.");
        }
        ...
        for (var i = 0; i < things.length; i++) {
            ...
            thing_def.thing(function (err, thing){
                ...
                name = thing.name;
                if (!name) {
                    return console.log("Error in register_from_config. The thing name is required");
                }
                
                model = thing.model;
                if (!model) {
                    return console.log("Error in register_from_config. The thing model is required");
                }
                ...
                if (!implementation) {
                    return console.log("Error in register_from_config. The thing implementation is required");
                }
                ...
                var isexists = get_thing(name);
                if (isexists) {
                    return console.log('The thing already exists: ' + name);
                }              
        ...
        console.log("Error in initializing things from local js file. " + e.message );
    }
}
