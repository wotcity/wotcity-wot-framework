//
// This code is ported from W3C Web of Things Framework for NodeJS
...
var Thing = exports.Thing = function Thing(name, model, implementation, remote) {
    ...
    this.remote_action = function (action, data) {
        ...
        client.post('/api/thing/action', params, function (err, req, res, data) {
            if (err) {
                console.log("/api/thing/action error: " + err);
            }            
            else if (!data && !data.result) {
                console.log("/api/thing/action error: invalid result");
            }
        });
    };
    ...
    this.remote_patch = function (property, value) {
        ...
        client.post('/api/thing/patch', params, function (err, req, res, data) {
            if (err) {
                console.log("/api/thing/patch error: " + err);
            }            
            else if (!data && !data.result) {
                console.log("/api/thing/patch error: invalid result");
            }
        });
    };
    ...
    this.property_get = function (property, callback) {
        if (this.isremote == false) {
            ...
            try {
                ...
                console.log('Error in populating remote property value: ' + e.message);
            }
        }
    };
                    ...
                    thing[action] = function (data) {
                        try {
                            if (thing.isremote == false) {
                                console.log('invoking action: ' + thing.name + '.' + action + '()');
                            ...
                            console.log('Error in invoking action: ' + e.message);
                        }
                    }
                })(act);
            }
    ...
    this.init_patch = function () {
        ...
        (function () {
            thing.patch = function (property, data) {
                try {
                    if (thing.isremote == false) {
                        console.log('Invoking patch handler. thing: ' + thing.name + ' property:' + property + '()');
                    ...
                    lconsole.log('Error in invoking patch: ' + e.message);
                }
            }
        })();        
    }
    ...
    this.register_proxy = function () {
        ...
        client.post('/api/endpoint/register', params, function (err, req, res, data) {
            if (err) {
                return console.log("Error in registering the remote proxy: " + err);
            }
            
            if (!data || !data.result) {
                console.log("Error in registering the remote proxy");
            }
            else {
                console.log("Remote proxy is registered for " + self.name);
            }
        });
    }   
    ...
    try {
        ...
        console.log("Error in initialsing thing: " + e.message);
    }
}
